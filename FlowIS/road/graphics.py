...
class LaneGraphics:
    ...
    def display(cls, lane: AbstractLane, surface: WorldSurface) -> None:
        ...
        # )
        # s_origin, _ = lane.local_coordinates(surface.origin)
        # s0_base = (int(s_origin) // cls.STRIPE_SPACING - stripes_count // 2) * cls.STRIPE_SPACING
        
        # for side in range(2):
        #     line_type = lane.line_types[side]
        #     # 基础偏移
        #     s0 = s0_base
        #     # 针对不同车道线类型调整偏移
...
class RoadGraphics:
    ...
    def display_traffic(
        ...
        for v in road.vehicles:
            VehicleGraphics.display(v, surface, offscreen=offscreen, draw_roof=True)
