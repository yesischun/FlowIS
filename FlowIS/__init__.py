...
def register(*, id: str, **kwargs):
    """Register env once; skip silently if already present."""
    if id in registry:
        return
...
def _register_highway_envs():
    ...
    register(
        id="HighDEnv-v0",
        entry_point="highway_env.envs.highway_highD_env:HighDEnv",
    )
    register(
        id="InDEnv-v0",
        entry_point="highway_env.envs.intersection_inD_env:InDEnv",
    )


def register_highway_envs():
    """Public registration hook used by Gymnasium plugin entry points."""
