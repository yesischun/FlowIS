    ...
    def _make_road(self) -> None:
        ...
        for corner in range(4):
            ...
            start = rotation @ np.array(
                [lane_width / 2, access_length + outer_distance-1]
            )
            ...
            end = rotation @ np.flip([lane_width / 2, outer_distance -1], axis=0)
            net.add_lane(
                "il" + str((corner - 1) % 4),
                "o" + str((corner - 1) % 4),
                StraightLane(
    ...
    def _make_vehicles(self, n_vehicles: int = 10) -> None:
        ...
        for ego_id in range(0, self.config["controlled_vehicles"]):
            ...
            try:
                ...
                pass
            ego_vehicle.color=(109, 43, 158)
            self.road.vehicles.append(ego_vehicle)
            self.controlled_vehicles.append(ego_vehicle)
            for v in self.road.vehicles:  # Prevent early collisions
                if (
                    v is not ego_vehicle
                    and np.linalg.norm(v.position - ego_vehicle.position) < 10
                ):
