...
class IntersectionEnv(AbstractEnv):
    ...
    def _make_vehicles(self, n_vehicles: int = 10) -> None:
        ...
        while True:
            ...
            # ttc=calculate_ttc(scene_behavior)
            # if ttc>0.5:
            if np.all((scene_behavior >= 0) & (scene_behavior <= 1)):
                scene_behavior=self.NDE_intersection['fit_scene_behavior'].inverse_transform(scene_behavior)[0]
                a=self.TTCP(scene_behavior)
                if a>3:
                    break
        # 初始场景的车道
